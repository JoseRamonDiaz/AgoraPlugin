 ...
 */
/**
 * Cadena para descarga http://www.agora.uady.mx/recurso/descargar/58 url para
 * 
 * descargar recursos con el id Cadena para visualizar por microsoft viewer,
 * para documentos office 
 * https://view.officeapps.live.com/op/view.aspx?src=http://www.agora.uady.mx/recurso/descargar/112
 * 
 * Usar el viewer de google para los demás archivos
 * 
 * Cambiar tamaño de cuadro de resultados de la búsqueda
 * 
 */
...
M.mod_agora.init = function (Y) {
    ...
    /*
     * 
     * @param {Integer} id @para {Float} leve
     * 
     */
    ...
    function fadein(id) {
        var level = 0;
        while (level <= 1) {
            // setTimeout( "gradient('" + id + "'," + level + ")", (level* 1000)
            // + 10);
            setTimeout(function () {
                gradient(id, level);
            }, (level * 1000) + 10);
     ...
     * 
     * @param {String} titulo @param {String} _url
     * 
     */
    ...
    function openbox(titulo, _url) {
        ...
        descarga.href = _url;
        var newUrl = 'http://docs.google.com/viewer?url=' + url
                + '&embedded=true';
     ...
     * 
     * @param {String} ruta @param {String} nombre
     */
    ...
    function restablecerLightBox() {
        var preview = Y.Node
                .create('<div id="preview"><iframe id="gframe" scrolling="auto"></iframe></div>');
        var box = Y.one('#box');
    ...
    function esExtensionValida(extensionArchivo) {
        ...
        if (extensionArchivo == "ppt" || extensionArchivo == "pdf"
                || extensionArchivo == "doc" || extensionArchivo == "odt"
     ...
     * 
     * @param {Object} enlaceRecurso @param {String} extension @param {Integer}
     * idRecurso @param {String} tituloReccurso @param {String} urlRecurso
     */

    function abrirLightBoxT(enlaceRecurso, extension, idRecurso, tituloRecurso,
            urlRecurso) {
        ...
        if (esExtensionValida(extension)) {
            ...
            enlaceSeleccionado.onclick = function () {
                return false;
            };
            return false;
    ...
    function mostrarResultadoT_Busqueda(recursos) {
        ...
        var botonAceptar = Y.Node
                .create('<input type="button" id="aceptarRecursoChecka" value= "Aceptar recurso"/>');
        ...
        if (recursos.length == 0) {
            ...
            Console.log('checkBox: ' + checkboxManager);
        }
    }
    ...
    function mostrarDetalleRecursoSeleccionado(_recursoSeleccionado) {
        ...
        var titulo = recursoSeleccionado.title == null ? "Sin titulo"
        ...
        var descripcion = recursoSeleccionado.description == null ? "Sin descripcion"
                : recursoSeleccionado.description;
        ...
        var disciplina = recursoSeleccionado.discipline == null ? "Sin desciplina"
                : recursoSeleccionado.discipline;
        ...
        if (esExtensionValida(extension) || extension == "swf") {
            ...
            var url = direccionServidor + 'recurso/ver/contenido/'
                    + recursoSeleccionado.id_recurso;
            var visualizar = Y.Node
                    .create('<input type="button" value="Visualizar"/>');
            visualizar.on("click", abrirLightBoxT, null, extension,
                    recursoSeleccionado.id_recurso, titulo, url);
            opciones.append(visualizar);
        }

        // var visualizar = Y.Node.create('<input type="button"
        // value="Visualizar"/>');
    ...
    /**
     * @param {int}
     *            idRecurso identificador del recurso seleccionado
     */
    function obtenerDetalleRecuro(idRecurso) {
        ...
        var urlDetalleRecurso = obtenerURLDetalleRecuros(idRecurso,
                direccionServidor);
        ...
        var configuracionPeticion = {
            ...
            on: {
                ...
                failure: function (o) {
                    alert('Error al obtener los datos del recurso 2');
    ...
    function mostrarTablaResultados(recursos) {
        ...
        var tabla = null;
        Y.Get.js(M.cfg.wwwroot + '/mod/agora/js_templates/table-template.js',
                function (err) {
                    ...
                    var output = Y.Handlebars.templates['table-template']({
                        recurso: recursos
                    });
                    Y.log(output);
                    console.log('output: ' + output);
                    output = replaceAll(output, '"checkbox"', '"radio" name="objAprendizaje"');
    ...
    function abrirVistaPreviaArchivo(element) {
        ...
        var extension = obtenerExt(idRecurso);
        console.log("extension: " + extension);
        // Link de descarga de contenido agora
        urlSeleccionada = 'http://www.agora.uady.mx/recurso/descargar/'
                + idRecurso;
        tituloRecursoSeleccionado = element.target.get("innerHTML");
        console.log('TituloSeleccionado: ' + tituloRecursoSeleccionado);
            ...
            on: {
                success: function (id, o, a) {
                    ...
                    try {
                        ...
                        // Y.log(recurso);
                        // esconderCamposBusqueda();
                        if (isGoogleExt(extension)) {
                            window.open('http://docs.google.com/viewer?url='
                                    + direccionRecurso + '&embedded=true');
                        }
                        if (isMicrosoftExt(extension)) {
                            window.open('https://view.officeapps.live.com/op/view.aspx?src=' + direccionRecurso);
                        }
                    } catch (e) {
                        alert('Error al obtener los datos del recurso 3');
                        Y.error(e);
                        mostrarCamposBusqueda();
                ...
                failure: function (o) {
                    alert('Error al obtener los datos del recurso 4');
                    mostrarCamposBusqueda();
                }
            }
        };
        ...
        Y.io.header('X-Requested-With');
        console.log('UrlDetRec: ' + urlDetalleRecurso);
        console.log('ConfigPet: ' + configuracionPeticion);
        var obj = Y.io(urlDetalleRecurso, configuracionPeticion);
    }

    function obtenerExt(idRecurso) {
        var eExtension = document.getElementById("ext_" + idRecurso);
        console.log("eExtensión" + eExtension);
        return eExtension.innerHTML;
    }

    function isGoogleExt(extension) {
        return extension === 'pdf';
    }

    function isMicrosoftExt(extension) {
    ...
    function obtenerFila(recurso) {
        ...
        var columnaCheckbox = Y.Node
                .create('<td><input type="radio" class="idRecurso" value="'
                        + recurso.id_recurso + 'ab' + '" 		id="check_'
                        + recurso.id_recurso + '"/></td>');

        var columnaImagen = crearColumnaImagen(recurso);

        var enlace = crearEnlace(recurso);

        var descripcionRecurso = recurso.descripcion == null ? "Sin descripcion"
                : recurso.descripcion;

        var descripcion = Y.Node.create('<br/>' + descripcionRecurso
                + '<br/>Extension: <span id="ext_' + recurso.id_recurso + '">'
                + recurso.extension + '</span>');
    ...
    /*
     * Crea un enlace para mostrar los recursos encontrados @param {Object}
     * recurso
     * 
     */
    function crearEnlace(recurso) {
        ...
        var urlRecurso = recurso.url_base + 'recurso/ver/contenido/'
                + recurso.id_recurso;
        ...
        var tituloRecurso = recurso.titulo == null ? "Sin t&iacute;tulo"
                : recurso.titulo;

        enlace.set("innerHTML", tituloRecurso);

        enlace.on("click", abrirLightBoxT, null, extension, recurso.id_recurso,
                recurso.titulo, urlRecurso);
    ...
    function crearColumnaImagen(recurso) {
        var enlace = Y.Node.create('<a><img src="' + recurso.icono
                + '" title="' + recurso.extension
    ...
    var handleStart = function (id, a) {
        ...
        // output.set("innerHTML", "<li>Loading news stories via Yahoo! Pipes
        // feed...</li>");
    }

    // Event handler for the success event -- use this handler to write the
    // fetched
    // RSS items to the page.
    var handleSuccess = function (id, o, a) {

        // We use JSON.parse to sanitize the JSON (as opposed to simply
        // performing an
        // JavaScript eval of the data):
        sResponse = o.responseText;
        cargaHashMap();
    ...
    var handleClick = function (o) {
        ...
        if (palabraBuscar.length <= 0) {
            ...
            var urlBusqueda = obtenerURLBusqueda(palabraBuscar,
                    direccionServidor);
    ...
    /*
     * Obtiene la url del servidor para realizar la busqueda @param {String}
     * palabraBuscar palabra que se desea buscar @param {String}
     * direccionServidor direccion actual del servidor @return url de busqueda
     */
     ...
     * @param {String} idRecurso @param {String} direccionServidor @return
     * Direccion para obtener los datos de un recurso
     */
    ...
    function aceptarRecurso(o) {
        ...
        if (checkSeleccionado == null) {
            ...
            urlRecurso.set("value", 'http://www.agora.uady.mx/recurso/descargar/' + _idRecurso);

            // Estableciendo el titulo del recurso
            var titulo = Y.one("#tituloRecurso");
            tituloRecursoSeleccionado = hmObjApren[_idRecurso];
            console.log('tituloRecusoSeleccionado: '+tituloRecursoSeleccionado);
